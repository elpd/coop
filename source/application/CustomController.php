...
class CustomController extends Zend_Controller_Action
{
    ...
    public function saveOrderAction()
    {
        ...
        $isSaveAction = !empty($post['save_form_submit']) ? true : false;
        $isCloseAction = !empty($post['close_order_submit']) ? true : false;

        // Request validation: only one type of action is allowed and one is needed. xor.
        if (!($isSaveAction xor $isCloseAction)) {
            throw new \Exception('only of type of action is allowed and one is needed.');
        }
        ...
        // Item edit check: only orders in unpayed (open) status can be edited.
        if (!empty($order) && $order['order_status'] != 'unpayed') {
            throw new \Exception('order in current status: ' . $order['order_status'] .
        ...
        $temp_order_total = $coop_orders->calcTotalPurchase($order['order_id']);

        $coop_orders->editOrder($order['order_id'], array(
            'order_last_edit' => date('Y-m-d H:i:s'),
            'order_total_when_closed' => $temp_order_total
        ));
        ...
        if ($isCloseAction) {
            ...
            $coop_orders->closeOrder($order['order_id'], $actual_payment);
        }
