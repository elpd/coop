<script>
    ...
    var user_current_debt = {if $user_current_debt}{$user_current_debt}{else}0{/if};
    var previous_debt_when_closed = {$previous_debt_when_closed};
    var previous_order_total_when_closed = {$previous_order_total_when_closed};
    ...
    <div id="order_form" class="list">
        <form action="{$public_path}/{$controller}/save-order" method="post">
            {if $duty_editing}
                ...
                &nbsp;&nbsp;
                <input style="position: relative; bottom: 3px;" size="60" name="user_comments" class="user_comments"
                       value="{$order.user_comments|escape:"html"|stripslashes}"/>
                <br/>
                <b>טיפ:</b>
                תורנ/ית יקר/ה, השתמש/י בקיצורי המקלדת Ctrl+1 לחיפוש/הוספת מוצר (ומשם אפשר לעשות Tab ואנטר)
                <br/>
                <br/>
            {/if}
            <table style="width: 930px;">
                ...
                <tr>
                    <td colspan="{if $order_view_type == "list"}7{else}4{/if}" class="colspan"
                        category_id="{$cat.category_id}">{$cat.category_name|escape:"html"|stripslashes}</td>
                </tr>
                ...
                <tr category_id="{$cat.category_id}" product_id="{$product.product_id}">
                    <td class="{$cellClass}"><a name="go_{$product.product_id}"
                                                {if $product.product_image || $product.product_about != null}class="order_name"
                                                hasImage="{$product.product_image}" href="javascript:void(0);" {/if}
                                                id="{$product.product_id}">{$product.product_name|escape:"html"|stripslashes}</a>
                    </td>
                    <td class="{$cellClass}"><input type="text" size="4" dir="ltr" maxlength="10" class="amount_input"
                                                    name="items[{$product.product_id}]"
                                                    product_id="{$product.product_id}"
                                                    value="{$items[$product.product_id].item_amount}"
                                                    original_amount="{$items[$product.product_id].item_amount}"
                                                    items_left="{if $product.product_items_left == "ללא הגבלה"}unlimited{else}{$product.product_items_left-$product.orders_count}{/if}"
                                                    {if !$allow_edit}disabled{/if} /> {$product.product_measure|escape:"html"|stripslashes}
                    </td>
                    <td class="{$cellClass}">₪<a class="price"
                                                 product_id="{$product.product_id}">{$product.product_price|escape:"html"|stripslashes}</a>
                    </td>
                    <td class="{$cellClass}">₪<a class="amount_txt"
                                                 product_id="{$product.product_id}">{$product.product_price * $items[$product.product_id].item_amount}</a>
                    </td>
                ...
                <tr>{/if}
                    <td class="gallery_item" category_id="{$cat.category_id}"
                        style="width: 215px; padding: 0; margin: 0; border: 1px solid black; vertical-align: text-top; text-align: center;">
                        <p class="title">
                            <a class="title{if $product.product_image || $product.product_about != null} order_name"
                               hasImage="{$product.product_image}" href="javascript:void(0);{/if}"
                               id="{$product.product_id}">{$product.product_name|escape:"html"|stripslashes}</a><br/>
                            {if $product.product_description != null}<a
                                    class="description">{$product.product_description|escape:"html"|stripslashes}</a>
                                <br/>
                            {/if}
                            <span class="show_manufacturer"><b>יצרן:</b> {$product.product_manufacturer|escape:"html"|stripslashes}</span><br/>
                            {if $product.product_image}<img
                            src="{$public_path}/user/view-product-image/id/{$product.product_id}"/>{/if}<br/>
                            <span class="show_items_left"><b>במלאי:</b> {if $product.product_items_left == "ללא הגבלה"}{$product.product_items_left}{else}
                                    <a class="items_left"
                                       id="{$product.product_id}">{$product.product_items_left-$product.orders_count}</a>
                            ...
                            <span class="show_amount"><b>כמות:</b> <input type="text" size="2" dir="ltr" maxlength="10"
                                                                          ...
                                                                          product_id="{$product.product_id}"
                                                                          value="{$items[$product.product_id].item_amount}"
                                                                          {if !$allow_edit}disabled{/if} /></span><br/>
                            <span class="show_total"><b>עלות:</b> ₪<a class="amount_txt"
                                                                      product_id="{$product.product_id}">{$product.product_price * $items[$product.product_id].item_amount}</a></span>
                        </p>
            ...
            {if $allow_edit && ((! isset($order)) || $order.order_status == "unpayed")}
                <div id="order_bottom_sticky">
                    ...
                    <div id="sticky_order_information_for_buyer">

                        {include file='common/order_revaluation.tpl'}

                        {if $duty_editing}
                            <br/>
                            <div id="sticky_cashier_input" class="cashier_input">
                                ...
                                <label for="estimated_debt_after_payment">חוב חדש:</label>
                                <input type="text" id="estimated_debt_after_payment"
                        ...
                        {include file='common/order_status.tpl'}
        ...
        {if ! $allow_edit || $order.order_status == "payed"}
            {if $duty_editing}
                <div id="goback_link">
                    <a href="{$public_path}/duty" tabindex="1"><< חזרה לרשימת ההזמנות</a>
                </div>
            ...
            {if $order.order_status == "payed"}
                <div id="order_information_when_closed">
                    <div>
                        <label for="closed_order_amount">קניה נוכחית:</label>
                        <input type="text" id="closed_order_amount"
                               class="presentational_input" value="{$previous_order_total_when_closed}"
                               readonly/>
                        &nbsp;<b>₪</b>
                        <label for="closed_previous_debt">חוב קודם:</label>
                        <input type="text" id="closed_previous_debt"
                               class="presentational_input" value="{$previous_debt_when_closed}"
                               readonly/>
                        &nbsp;<b>₪</b>
                        <label for="closed_overall_amount_to_pay">לתשלום:</label>
                        <input id="closed_overall_amount_to_pay"
                               class="presentational_input"
                               value="{$previous_order_total_when_closed + $previous_debt_when_closed}"
                               readonly/>
                        &nbsp;<b>₪</b>
                    ...
                    <div id="order_payment_information">
                        <label for="transfer_payment_amount">סכום התשלום שבוצע על ההזמנה:</label>
                        <input type="text" id="transfer_payment_amount"
                               class="presentational_input" value="{$connected_transfer_amount}"
                               readonly/>
                        &nbsp;
                        <b>₪</b>
                    </div>
            ...
            {else}
                <div id="order_information_for_buyer">
                    {include file='common/order_revaluation.tpl'}
                    {include file='common/order_status.tpl'}
                </div>
            {/if}
